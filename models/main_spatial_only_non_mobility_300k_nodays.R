# Model definition
description = "No mobility indicators and no DaysSince10Deaths"
...
# Drop mobility indicators
mobility_cols = c("RetailAndRecPctChg", "GroceryPharmacyPctChg", "ParksPctChg", "TransitPctChg", "WorkplacesPctChg", "ResidentialPctChg")
no_mobility = df[,!(names(df) %in% mobility_cols)]
# Drop response, non-numeric variables
no_mobility = no_mobility[,!(names(no_mobility) %in% c("FutureDeathsPer100k", "DaysSince10Deaths", "Date","StateName","StateID"))]
# Convert dataset to matrix form
X = data.matrix(no_mobility)
